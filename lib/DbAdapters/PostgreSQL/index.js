...
function PostgreSQLAdapter(config) {
  ...
  let getConnection = (0, _poolConnectionGetter2.default)(_extends({
    host,
    port,
    database,
    user,
...
const prefix = (0, _utils.prefixedString)('[grpc Event Store PostgreSQLAdapter] ');
const validateConfig = ({
  ...
  if (!(0, _lodash.isString)(password) || (0, _lodash.isEmpty)(password)) throw new TypeError(prefix(`config.password MUST be a non empty string. Received ${password}`));
  ...
  if (ssl !== undefined) {
    ...
    if (!(0, _lodash.isObject)(ssl)) throw new TypeError(eMsg);
    if (!(0, _lodash.isString)(ssl.ca) || (0, _lodash.isEmpty)(ssl.ca)) throw new TypeError(eMsg);
    if (!(0, _lodash.isString)(ssl.cert) || (0, _lodash.isEmpty)(ssl.cert)) throw new TypeError(eMsg);
    if (!(0, _lodash.isString)(ssl.key) || (0, _lodash.isEmpty)(ssl.key)) throw new TypeError(eMsg);
  }
};

let createTableSQL;
Object.defineProperty(PostgreSQLAdapter, 'createTableSQL', {
  value: (tableName = 'events') => {
    if (!createTableSQL) {
      createTableSQL = _fs2.default.readFileSync(_path2.default.resolve(__dirname, 'createTable.sql'), 'utf8');
    }
    return createTableSQL.replace(/events/g, tableName);
  },
  enumerable: true
});
