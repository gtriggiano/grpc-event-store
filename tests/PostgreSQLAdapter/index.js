...
const PostgreSQLAdapter = require(`${libFolder}/DbAdapters/PostgreSQL`).default
const {
  ANY_VERSION_NUMBER,
  ANY_POSITIVE_VERSION_NUMBER
} = require(`${libFolder}`)

const getAdapter = () => PostgreSQLAdapter({host: 'postgresql', user: 'eventstore', password: 'password'})

describe('PostgreSQLAdapter([config])', () => {
  it('is a function', () => should(PostgreSQLAdapter).be.a.Function())
  it('PostgreSQLAdapter.createTableSQL([tableName]) is a function', () => should(PostgreSQLAdapter.createTableSQL).be.a.Function())
  it('PostgreSQLAdapter.createTableSQL([tableName]) returns a `CREATE TABLE` sql query', () => {
    let defaultSQL = PostgreSQLAdapter.createTableSQL()
    let customSQL = PostgreSQLAdapter.createTableSQL('mytable')
  ...
  it('throws if config.host is not a non empty string', () => {
    should(() => {
      PostgreSQLAdapter({host: ''})
    }).throw(new RegExp('config.host MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({host: 3})
    }).throw(new RegExp('config.host MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({host: 'test'})
    }).not.throw()
  })
  it('throws if config.port is not a positive integer', () => {
    should(() => {
      PostgreSQLAdapter({port: 0})
    }).throw()
    should(() => {
      PostgreSQLAdapter({port: ''})
    }).throw()
    should(() => {
      PostgreSQLAdapter({port: 1000})
    }).not.throw()
  })
  it('throws if config.database is not a non empty string', () => {
    ...
    should(() => {
      PostgreSQLAdapter({database: 3})
    }).throw(new RegExp('config.database MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({database: 'test'})
    }).not.throw()
  })
  it('throws if config.table is not a non empty string', () => {
    should(() => {
      PostgreSQLAdapter({table: ''})
    }).throw(new RegExp('config.table MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({table: 3})
    }).throw(new RegExp('config.table MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({table: 'test'})
    }).not.throw()
  })
  it('throws if config.user is not a non empty string', () => {
    should(() => {
      PostgreSQLAdapter({user: ''})
    }).throw(new RegExp('config.user MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({user: 3})
    }).throw(new RegExp('config.user MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({user: 'test'})
    }).not.throw()
  })
  it('throws if config.password is not a non empty string', () => {
    should(() => {
      PostgreSQLAdapter({password: ''})
    }).throw(new RegExp('config.password MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({password: 3})
    }).throw(new RegExp('config.password MUST be a non empty string'))
    should(() => {
      PostgreSQLAdapter({password: 'test'})
    }).not.throw()
  })
  it('throws if config.maxPoolClients is not a positive integer', () => {
    should(() => {
      PostgreSQLAdapter({maxPoolClients: ''})
    }).throw(new RegExp('config.maxPoolClients MUST be a positive integer'))
    should(() => {
      PostgreSQLAdapter({maxPoolClients: 0})
    }).throw(new RegExp('config.maxPoolClients MUST be a positive integer'))
    should(() => {
      PostgreSQLAdapter({maxPoolClients: 10})
    }).not.throw()
  })
  it('throws if config.minPoolClients is truthy and is not both a positive integer and < config.maxPoolClients', () => {
    should(() => {
      PostgreSQLAdapter({minPoolClients: 'x'})
    }).throw(new RegExp('config.minPoolClients MUST be a positive integer lower than config.maxPoolClients'))
    should(() => {
      PostgreSQLAdapter({minPoolClients: -1})
    }).throw(new RegExp('config.minPoolClients MUST be a positive integer lower than config.maxPoolClients'))
    should(() => {
      PostgreSQLAdapter({maxPoolClients: 5, minPoolClients: 10})
    }).throw(new RegExp('config.minPoolClients MUST be a positive integer lower than config.maxPoolClients'))
    should(() => {
      PostgreSQLAdapter({minPoolClients: 2})
    }).not.throw()
  })
  it('throws if config.idleTimeoutMillis is not an integer >= 1000', () => {
    should(() => {
      PostgreSQLAdapter({idleTimeoutMillis: ''})
    }).throw(new RegExp('config.idleTimeoutMillis MUST be a positive integer higher then 999'))
    should(() => {
      PostgreSQLAdapter({idleTimeoutMillis: 100})
    }).throw(new RegExp('config.idleTimeoutMillis MUST be a positive integer higher then 999'))
    should(() => {
...
describe('db = PostgreSQLAdapter(config)', () => {
  it('db.getEvents() is a function', () => should(PostgreSQLAdapter().getEvents).be.a.Function())
  it('db.getEventsByStream() is a function', () => should(PostgreSQLAdapter().getEventsByStream).be.a.Function())
  it('db.getEventsByStreamsCategory() is a function', () => should(PostgreSQLAdapter().getEventsByStreamsCategory).be.a.Function())
  it('db.appendEvents() is a function', () => should(PostgreSQLAdapter().appendEvents).be.a.Function())
})
